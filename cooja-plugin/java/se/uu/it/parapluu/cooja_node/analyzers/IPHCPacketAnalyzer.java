...
public class IPHCPacketAnalyzer extends PacketAnalyzer {
  ...
  @Override
    ...
    LinkedList<OtpErlangObject> iphc_opts = new LinkedList<OtpErlangObject>();
    
    iphc_opts.add(PacketAnalyzer.make_opt("tf", new OtpErlangInt(tf)));
    iphc_opts.add(PacketAnalyzer.make_opt("nh", nhc ? new OtpErlangAtom("compressed") : new OtpErlangAtom("inline")));
    iphc_opts.add( PacketAnalyzer.make_opt("hlim", hlim == 0 ? new OtpErlangAtom("inline") : new OtpErlangInt(hlim)));
    iphc_opts.add(PacketAnalyzer.make_opt("cid", new OtpErlangInt(cid)));
    iphc_opts.add(PacketAnalyzer.make_opt("sac", sac == 0 ? new OtpErlangAtom("stateless"): new OtpErlangAtom("stateful")));
    iphc_opts.add(PacketAnalyzer.make_opt("sam", new OtpErlangInt(sam)));
    iphc_opts.add(PacketAnalyzer.make_opt("mcast", new OtpErlangAtom(m)));
    iphc_opts.add(PacketAnalyzer.make_opt("dac", dac == 0 ? new OtpErlangAtom("stateless"): new OtpErlangAtom("stateful")));
    iphc_opts.add(PacketAnalyzer.make_opt("dam", new OtpErlangInt(dam)));
    
    
    if (cid == 1) {
    	PacketAnalyzer.make_opt("sci", new OtpErlangInt(packet.get(2) >> 4));
    	PacketAnalyzer.make_opt("dci", new OtpErlangInt(packet.get(2) & 0x0f));
    	sci = packet.get(2) >> 4;
...
//        }
	...
	if (proto == PROTO_UDP) {
      OtpErlangTuple sport = PacketAnalyzer.make_opt("srcport", new OtpErlangInt(srcPort));
      OtpErlangTuple dport = PacketAnalyzer.make_opt("srcport", new OtpErlangInt(destPort));
      OtpErlangObject[] objs = { protocol, tc, fl, from, to, sport, dport };
  	iphc_opts.add(PacketAnalyzer.make_opt("ipv6", new OtpErlangList(objs)));
    } else {
